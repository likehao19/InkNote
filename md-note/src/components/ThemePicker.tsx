...
function ThemePreviewMock({ theme, locale }: { theme: "light" | "dark"; locale: Locale }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    applyCustomCssToHost(el);
    const onChange = () => applyCustomCssToHost(el);
    window.addEventListener("mdnote-custom-css-changed", onChange);
    return () => {
      window.removeEventListener("mdnote-custom-css-changed", onChange);
      removeCustomCssFromHost(el);
    };
  }, [theme]);

  return (
    <div ref={ref} className="theme-preview-mock" data-theme={theme}>
