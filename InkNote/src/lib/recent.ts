...
export function remapRecentFiles(oldPath: string, newPath: string) {
  const list = getRecentFiles().map((path) => remapPath(path, oldPath, newPath));
  setStoredValue(KEY, JSON.stringify([...new Set(list)]));
}

export function removeRecentFilesUnder(path: string) {
  const list = getRecentFiles().filter((item) => !isPathUnder(item, path));
  setStoredValue(KEY, JSON.stringify(list));
}
