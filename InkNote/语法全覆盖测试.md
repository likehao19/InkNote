---
title: InkNote Markdown 功能参考文档
author: 测试作者
date: 2026-08-18
tags: markdown, test, mdnote
description: InkNote 支持的 Markdown 语法、组件与排版能力参考
---

# InkNote Markdown 功能参考

> 本文档系统展示 InkNote 支持的 Markdown 渲染与编辑能力，可作为实时预览、源码模式、导出排版和组件操作的使用参考。

InkNote 基于 **Tauri 2** + **CodeMirror 6** 构建，提供专注、连贯的 Markdown 写作体验。下面按模块依次介绍标题、行内格式、列表、引用、代码、表格、数学公式、Mermaid 图表和脚注等能力。
...
[InkNote 项目主页](https://github.com/likehao19/InkNote) 是标准 Markdown 链接。
...
![InkNote 应用图标](./assets/app-icon.svg)
...
InkNote 支持脚注引用语法[^footnote-demo]。
...
下面三段用于测试中文字数、英文单词数与阅读时间估算。第一段以中文为主：
 ...
 Markdown 是一种轻量级标记语言，由约翰·格鲁伯于 2004 年创建，旨在让人们使用易读易写的纯文本格式编写文档，再转换成有效的 HTML。与传统富文本编辑器不同，Markdown 的源文件可以直接用任何文本编辑器打开，也非常适合版本控制系统追踪变更。近年来，Obsidian、Typora、Notion 等工具推动了「所见即所得」或「即时渲染」的书写体验，让 Markdown 从程序员的 README 专属格式，变成了知识管理、技术写作和日常笔记的主流选择。InkNote 保留 Markdown 的开放性与可移植性，同时通过 Tauri 提供原生窗口、文件系统访问和轻量的资源占用。
...
第三段中英混合，并包含数字与标点：
 ...
 现代笔记软件需要兼顾 **instant preview**、**low latency** 和 **native file access**。一款成熟的编辑器还需要处理好长文档性能（Performance）、数学公式（KaTeX）、代码高亮（highlight.js）以及国际化（i18n），这些也是 InkNote 持续完善的方向。
...
以上内容覆盖 InkNote 的核心 Markdown 语法和组件能力，可在实时预览与源码模式之间切换查看，并用于确认导出文档的排版效果。
